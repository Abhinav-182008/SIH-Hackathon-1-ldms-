...
def seed():
    ...
    with connect() as db:
        ...
        CREATE TABLE IF NOT EXISTS case_extractions(
            ...
            case_id TEXT NOT NULL REFERENCES cases(id), source_sha256 TEXT NOT NULL,
            requested_by TEXT NOT NULL REFERENCES users(id), created REAL NOT NULL,
            model TEXT NOT NULL, fields TEXT NOT NULL, status TEXT NOT NULL CHECK(status IN ('draft','verified','discarded')),
            confirmed_by TEXT REFERENCES users(id), confirmed_at REAL);
        CREATE INDEX IF NOT EXISTS extractions_document ON case_extractions(document_id,created);
    ...
    def dispatch(self, mutation):
        ...
        try:
            ...
            if path.startswith('/api/cases/') and not mutation:
                ...
                self.respond(200, c)
                return
            feature = re.fullmatch(r'/api/documents/([^/]+)/(integrity|extractions)(?:/([^/]+)/(edit|confirm|discard))?', path)
            if feature:
                did, operation, eid, action = feature.groups()
                did = unquote(did)
                db.execute('BEGIN IMMEDIATE')
                ...
                if operation == 'integrity':
                    require(mutation and not eid, 405, 'POST required')
                    verified = hmac.compare_digest(hashlib.sha256(row['body']).hexdigest(), row['sha256'])
                    audit(db, uid, 'Integrity verification ' + ('Verified' if verified else 'Verification Failed') + ' document ' + did, cid)
                    db.commit()
                    self.respond(200, {'verified': verified, 'sha256': row['sha256']})
                    return
                if not mutation:
                    require(not eid, 404, 'Not found')
                    records = [dict(r) for r in db.execute("SELECT * FROM case_extractions WHERE document_id=? AND (status='verified' OR (status='draft' AND requested_by=?)) ORDER BY created DESC", (did, uid))]
                    for record in records:
                        record['fields'] = json.loads(record['fields'])
                    db.commit()
                    self.respond(200, {'records': records, 'canConfirm': assigned(db, uid, cid) or reviewer(db, uid, cid)})
                    return
                if eid:
                    draft = db.execute('SELECT * FROM case_extractions WHERE id=? AND document_id=?', (eid, did)).fetchone()
                    require(draft is not None and draft['requested_by'] == uid)
                    require(draft['status'] == 'draft', 409, 'This draft is no longer editable')
                    if action == 'discard':
                        db.execute("UPDATE case_extractions SET status='discarded',fields='{}' WHERE id=?", (eid,))
                        audit(db, uid, 'AI draft discarded ' + eid + ' document ' + did, cid)
                    else:
                        require(hmac.compare_digest(draft['source_sha256'], row['sha256']) and hmac.compare_digest(hashlib.sha256(row['body']).hexdigest(), draft['source_sha256']), 409, 'Source integrity changed; confirmation blocked')
                        try:
                            values = ai_extraction.validate_fields(data.get('fields'))
                        except ValueError:
                            raise ApiError(400, 'Invalid extraction fields')
                        if action == 'confirm':
                            require(assigned(db, uid, cid) or reviewer(db, uid, cid), 403, 'An assigned officer or reviewer must confirm information')
                        if values != json.loads(draft['fields']):
                            audit(db, uid, 'Extracted data edited by user ' + eid + ' document ' + did, cid)
                        db.execute('UPDATE case_extractions SET fields=? WHERE id=?', (json.dumps(values), eid))
                        if action == 'confirm':
                            db.execute("UPDATE case_extractions SET status='verified',confirmed_by=?,confirmed_at=? WHERE id=?", (uid, time.time(), eid))
                            audit(db, uid, 'Extracted information confirmed ' + eid + ' document ' + did, cid)
                    db.commit()
                    self.respond(200, {})
                    return
                audit(db, uid, 'AI extraction requested document ' + did, cid)
                db.commit()  # Never hold SQLite's write lock during network calls.
                try:
                    if not hmac.compare_digest(hashlib.sha256(row['body']).hexdigest(), row['sha256']):
                        raise ai_extraction.Unavailable()
                    values, model, cleanup_failed = ai_extraction.extract(row)
                    ai_extraction.validate_fields(values)
                except Exception:
                    audit(db, uid, 'AI extraction failed document ' + did, cid)
                    db.commit()
                    raise ApiError(503, 'AI extraction unavailable')
                db.execute('BEGIN IMMEDIATE')
                # Grants/session may have expired or been revoked during the external call.
                current = db.execute('SELECT * FROM documents WHERE id=?', (did,)).fetchone()
                still_signed_in = db.execute('SELECT 1 FROM sessions WHERE token=? AND expires>?', (session['token'], time.time())).fetchone()
                if not still_signed_in or not can_read(db, uid, cid):
                    audit(db, uid, 'AI extraction failed (access changed) document ' + did, cid)
                    db.commit()
                    raise ApiError(403, 'Access denied')
                if not current or current['sha256'] != row['sha256'] or current['body'] != row['body']:
                    audit(db, uid, 'AI extraction failed (source changed) document ' + did, cid)
                    db.commit()
                    raise ApiError(409, 'Source integrity changed; extraction discarded')
                eid = 'EXT-' + secrets.token_hex(12)
                db.execute("INSERT INTO case_extractions(id,document_id,case_id,source_sha256,requested_by,created,model,fields,status) VALUES(?,?,?,?,?,?,?,?, 'draft')",
                    (eid, did, cid, row['sha256'], uid, time.time(), model, json.dumps(values)))
                audit(db, uid, 'AI extraction completed document ' + did + ' draft ' + eid, cid)
                if cleanup_failed:
                    audit(db, uid, 'AI temporary file cleanup failed document ' + did, cid)
                db.commit()
                self.respond(201, {'id': eid, 'status': 'draft', 'fields': values,
                    'cleanupWarning': 'Temporary xAI file deletion could not be confirmed. Contact the API account administrator.' if cleanup_failed else None})
                return
